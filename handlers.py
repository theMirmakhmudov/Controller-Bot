...
async def new_chat_member_answer(message: types.Message):
    for new_chat_member in message.new_chat_members:
        await message.answer_photo(
            ...
            f"Assalomu Aleykum, {new_chat_member.mention_html()}\nGuruhimizga Xush kelibsiz!")
...
async def left_chat_member_answer(message: types.Message):
    await message.answer_photo(
        ...
        f"Xayir, {message.left_chat_member.mention_html()}")
...
async def unmute_member_answer(message: types.Message):
    ...
    await message.chat.restrict(user_id, permissions)
    await message.answer_photo(
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQlPQKfIi9UUnwcNasWrtRXuZGvu1pUq1_XRw&usqp=CAU",
        f"✅{message.reply_to_message.from_user.mention_html()} endi yoza olasiz ❗️️")


async def ban_member_answer(message: types.Message):
    user_id = message.reply_to_message.from_user.id
    await message.chat.ban_sender_chat(user_id)
    await message.answer(f"{message.reply_to_message.from_user.mention_html()} guruhdan chetlashtirildi ❗️")
...
async def unban_member_answer(message: types.Message):
    ...
    await message.answer(
        f"{message.reply_to_message.from_user.mention_html()} guruhga qayta qo'shilish uchun ruxsat berildi ❗️")
